...
[![pypi](https://img.shields.io/badge/pypi-0.14.2-blue?style=for-the-badge&logo=pypi&logoColor=white)](https://pypi.org/project/kntgraph/)
...
| Version | Highlights |
| --- | --- |
| **0.14.2** *(current)* | Pyright: 5 pre-existing errors closed (UUID cast in role_systems, `bytes` accepted in `PipelineLike.set`, Protocol body uses `...`, obsolete `# type: ignore` removed). P4 (ADR-068 §3.4) shipped — incremental cache refresh via parallel `<key>:fold_cursor` key, payload stays legacy-compatible. `CacheWarmer.pump_once` always uses the warm path; no Redis I/O on the Consolidator tick. |
