...
def _current_tag() -> str:
    """Return the latest annotated tag, stripped
    of the leading ``v``. Fails with a non-zero
    exit if no tag exists.
    """
    ...
    if result.returncode != 0:
        raise SystemExit(
            "bump_version: 'git describe --tags "
            "--abbrev=0' failed; the repository has "
            "no annotated tags. Run "
            "'git tag -a vX.Y.Z' first, or apply the "
            "retroactive tags from ADR-051 PR 1."
