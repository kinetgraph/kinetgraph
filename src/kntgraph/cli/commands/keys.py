...
@app.command()
def generate(
    agent_id: str = typer.Option(..., "--agent-id", help="The ID of the Agent for which to generate the keys."),
    out_dir: str = typer.Option(None, "--out-dir", help="Directory to save the PEM files. If empty, prints to stdout."),
):
    """
    ...
    if out_dir:
        ...
        console.print(f"[green]Success![/green] Keys generated for agent '{agent_id}' at {out_path}")
    else:
