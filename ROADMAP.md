...
|---|---|---|---|---|---|
| 1 | `RuleBasedChatSystem._persona_for_view` corrigido — personas globs casam desde v0.9 | [ADR-061 §11.1](./ADRs/ADR-061-litellm-integration-review.md) | bug | Not started | — |
...
6. **`project_memory` composition in production dispatcher** (bug latente). **Status:** crítico. **Found:** 2026-08-26. O `ReactiveDispatcher` em produção **não chama** `project_memory`; o `_fold_with_filter` em `src/kntgraph/runner/_folding.py` faz apenas `default fold + overlay`. Consequência: `SessionComponent`, `ProfileComponent`, `ContinuityComponent` **nunca chegam** ao `AgentView` em produção. Os 5 role systems LLM-backed (`ChatRoleSystem`, `PlannerRoleSystem`, `SummarizerRoleSystem`, `PersonalizedRoleSystem`, e qualquer persona-via-`RoleComponent`) **não funcionam** em produção — eles leem `SessionComponent` e silenciosamente fazem `continue` (linha 121 do `_base.py`). Os 15 testes em `tests/agents/unit/roles/test_role_systems.py` e os exemplos 05b/05c **funcionavam via shim** (monkey-patch de `_fold_with_filter`) que simulava o comportamento que produção não tem. **Resolução dos testes:** apagados em 2026-08-26 (zero cobertura de role systems até o framework ser corrigido). **Resolução do bug:** abrir ADR novo ("Project Memory composition in production dispatcher") antes de reintroduzir os testes. **Tracking:** aguardando ADR; owner = core team.
