...
This ADR intentionally does **not** codify a unit-test
strategy that mocks the GLiNER2 backend itself. In the
knt project, model-backed behaviour belongs to the
real-integration suite; the framework tests should
exercise the registry/facade contract and the tool
composition, not assert on `GLiNER2.from_pretrained`
call counts or other mock-only behaviour.
...
This project prefers real behaviour over mock-heavy
unit tests when the external system is available in CI.
For GLiNER2-backed work, the policy is:

- Framework unit tests validate the contract at the
  framework boundary (registry caching, facade wiring,
  schema validation, error propagation, and Tool
  adaptation), without asserting on the GLiNER2 model
  internals.
- GLiNER2 itself stays in the real integration suite,
  where `extract_json` and the upstream model behaviour
  are exercised against a real model binary.
- Tests do not rely on `GLiNER2.from_pretrained` mock
  assertions as the primary signal of correctness.
  That would test the mock, not the production
  behaviour.

Concretely:

- The three existing adapters' `__init__` tests no
  longer treat `from_pretrained` as the behavioural
  target; they assert the registry/facade contract and
  the model-instance reuse semantics instead.
- New unit tests for `GlinerStructuredAdapter`
  inject a fake backend or a lightweight adapter stub
  and validate the adapter's extraction contract and
  field filtering. They do not depend on mocking the
  upstream model as the main assertion.
...
- New unit tests for `StructuredExtractionTool`
  use a fake `SLMStructuredExtractor` (the pattern is
  identical to
  `tests/unit/knowledge/extraction/test_argument_framework.py`).
- A new registry test asserts that two adapters with
  the same `(model_name, device)` receive the same
  instance, and two adapters with different keys
  receive different instances.
- A dedicated integration test file exercises the real
  GLiNER2 `extract_json` path against the actual model
  and schema dialect, guarded behind the optional
