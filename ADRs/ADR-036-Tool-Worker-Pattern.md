...
1. Ler o stream de eventos buscando por `tool.requested` (usando Redis Consumer Groups para paralelismo e confiabilidade).
...
4. Emitir um evento `tool.completed` (se `Ok`) ou `tool.failed` (se `Err`) no stream original do agente.
...
* **Alta Tolerância a Falhas:** Se um Worker "crashar" fazendo I/O, o Redis Consumer Group redistribui o `tool.requested` para outro Worker após o timeout. O *state* do domínio não sofre corrupção.
* **Métricas Claras:** Tempo de I/O torna-se a diferença entre o timestamp do `tool.requested` e do `tool.<name>.completed` no EventLog (campo `latency_ms` no `ToolCallCompletion`).
...
- **Framework**:
  ...
  - `core/world/projection_tool_calls.py` — `overlay_tool_calls`
    ...
    Suporta `tool.completed`/`tool.failed` (legado ADR-034) e
    `tool.<name>.completed`/`tool.<name>.failed` (WorkerManager).
    Removido `MappingProxyType` redundante (dataclass frozen
    já garante imutabilidade).
...
1. **Migrar `fmh_app/src/fmh_app/systems/geoencoder_system.py:94`**
   ...
   `tool.requested` para `ibge_geoencoder`; mover a chamada
...
5. **ADR-037 (proposto, não escrito): bug no `Event.create`**
   ...
   o `correlation_id` quebra em tool.requested → tool.completed
...
* Os ECS components `ToolCallRequest` e `ToolCallCompletion` já foram definidos no **ADR-034**, fornecendo o modelo de dados subjacente. ADR-036 mantém compatibilidade com a forma `tool.<name>.completed` (WorkerManager) e adiciona suporte a `tool.completed` (legado) via prefix-match em `overlay_tool_calls`.
