...
def test_knt_init_with_http(tmp_path: Path):
    ...
    try:
        ...
        main_content = (
            tmp_path / "my_app_http" / "src" / "my_app_http" / "main.py"
        ).read_text()
        assert (
            "from fastapi import FastAPI" not in main_content
        )  # create_app abstracts this
...
def test_knt_init_with_external_routing_mode(tmp_path: Path):
    ...
    assert (
        project_dir / "src" / "my_app_external" / "routing" / "adapters" / "external.py"
    ).is_file()
    assert (
        project_dir / "src" / "my_app_external" / "routing" / "components.py"
    ).is_file()
...
def test_knt_init_with_collaborate_routing_mode(tmp_path: Path):
    ...
    assert (
        project_dir / "src" / "my_app_collab" / "routing" / "coordinator.py"
    ).is_file()
