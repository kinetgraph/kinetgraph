...
# ---------------------------------------------------------------------------
# LiteLLMTransportAdapter: exception translation
# ---------------------------------------------------------------------------
# These tests verify the real `LiteLLMTransportAdapter` (not the
# fake) translates `litellm`'s native exceptions into the
# typed `LLMError` hierarchy. The translation is what makes
# the fallback loop in `LiteLLMTool.invoke()` work in
# production — without it, real `RateLimitError` from the
# provider would fall into the generic `except Exception`
# branch and the tool would not try the next model in
# `fallback_models`.


class _LitellmExc:
    """
    Test helper: a stand-in for `litellm.*Error` classes.
    We don't import `litellm` directly because the
    `LiteLLMTransportAdapter` does a `try/except` chain on
    `litellm.RateLimitError`, `litellm.AuthenticationError`,
    and `litellm.APIError`. To exercise the chain without a
    real `litellm` provider, we patch the transport's
    `acompletion` to raise a real `litellm` exception
    instead. The `litellm` package is required at test
    time (it's a dependency of `fmh-agents`).
    """
...
class TestLiteLLMTransportAdapterExceptionTranslation:
    ...
    respectively. The fallback loop in
    `LiteLLMTool.invoke()` uses `isinstance` checks on
    these typed exceptions, so the translation is
    load-bearing.
    """
