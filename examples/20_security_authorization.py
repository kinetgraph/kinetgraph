...
async def main() -> None:
    ...
    try:
        ...
        agent_principal = Principal.from_agent_id(
            "tenant-A.agent-1", role=Role.agent, key_id="key-001"
        )
        ...
        allowed = policy.allows(
            principal=agent_principal, resource=res_own, action=Action.write
        )
        print(
            f"Agent writing to own tenant-A.agent-1: {'ALLOWED' if allowed else 'DENIED'}"
        )

        # Test 2: Agent trying to write an event to another tenant
        res_other = Resource(kind="event", tenant_id="tenant-B")
        allowed = policy.allows(
            principal=agent_principal, resource=res_other, action=Action.write
        )
        ...
        admin_principal = Principal(
            agent_id="admin-1", role=Role.admin, tenant_id=None, key_id="key-002"
        )

        # Test 3: Admin trying to write an event to tenant-B
        allowed = policy.allows(
            principal=admin_principal, resource=res_other, action=Action.write
        )
        ...
        allowed = policy.allows(
            principal=admin_principal, resource=res_system, action=Action.administer
        )
        ...
        print(
            f"Bound Context Principal: {current_principal.agent_id} (Role: {current_principal.role.value})"
        )
