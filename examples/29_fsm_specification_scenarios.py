# SPDX-FileCopyrightText: 2026 kinetgraph
#
# SPDX-License-Identifier: Apache-2.0

"""
Example 29: FSM State Transition Guards with Composable Specification Pattern (ADR-069 §2 & §3).

Demonstrates FSM state transitions guarded by composable Specifications (built-in and custom):

Scenario 1: FSM Parallel Multi-Tool Fast-Fail with Specification Guard
  - Transition 'created' -> 'processing' guarded by:
      (DomainStateIs("stage", "created") OR DomainStateIs("stage", "draft")) AND OrderAmountValid(10, 1000)
  - Dispatches 3 parallel tools (fast success, fast failure, slow 10s tool).
  - Fast failure triggers 'rejected' transition in ~0.1s without waiting for 10s tool.

Scenario 2: FSM Interleaved Non-Blocking Tools with Specification Guard
  - Transitions guarded by OrderAmountValid(10, 500) and DomainStateIs("stage", "executing").
  - Fast tool completes at t < 0.5s transitioning state to 'partially_completed' while 3s tool runs in background.

Run with:
  KNT_REDIS_FAKE=1 uv run python examples/29_fsm_specification_scenarios.py
"""

from __future__ import annotations

import asyncio
import os
import time
from dataclasses import dataclass
from typing import Any

from kntgraph.concordos import Composable, Specification, StepContext
from kntgraph.concordos.fsm import (
    BusinessFSMConcordo,
    FSMConfig,
    FSMTransition,
)
from kntgraph.concordos.specs import DomainStateIs
from kntgraph.core.event import Event, correlation_middleware
from kntgraph.core.result import Err, Ok, Result, ToolError
from kntgraph.core.world import DomainComponent, World, domain_component
from kntgraph.infra.redis._event_log import RedisEventLogAdapter
from kntgraph.runner.reactive import ReactiveDispatcher
from kntgraph.stream.event_log import EventLog
from kntgraph.tools import tool_worker
from kntgraph.tools.manager import WorkerManager
from kntgraph.tools.router import ToolRouter

# ===========================================================================
# Custom Specification Definition
# ===========================================================================


@dataclass(frozen=True, slots=True)
class OrderAmountValid(Specification, Composable):
    """Custom Specification evaluating whether domain order_amount is within range."""

    min_amount: int
    max_amount: int

    def is_satisfied_by(self, ctx: StepContext) -> bool:
        if ctx.domain is None:
            return False
        amount = getattr(ctx.domain, "order_amount", 0)
        return self.min_amount <= amount <= self.max_amount


# ===========================================================================
# SCENARIO 1: Multi-Tool Fast-Fail FSM Components & Specification Guard
# ===========================================================================


@domain_component("order.submit")
@dataclass(frozen=True, slots=True)
class FSMSpecOrderComponent(DomainComponent):
    stage: str = "created"
    order_amount: int = 250


@tool_worker(name="ex29_fast_success_tool", max_concurrency=10, retries=0)
class SpecFastSuccessTool:
    async def invoke(
        self,
        *,
        idempotency_key: str,
        state: str = "",
        **kwargs: Any,
    ) -> Result[dict, ToolError]:
        return Ok({"status": "processed"})


@tool_worker(name="ex29_fast_failure_tool", max_concurrency=10, retries=0)
class SpecFastFailureTool:
    async def invoke(
        self,
        *,
        idempotency_key: str,
        state: str = "",
        **kwargs: Any,
    ) -> Result[dict, ToolError]:
        return Err(ToolError("Simulated fast failure in spec test"))


@tool_worker(name="ex29_slow_10s_tool", max_concurrency=10, retries=0)
class SpecSlow10sTool:
    async def invoke(
        self,
        *,
        idempotency_key: str,
        state: str = "",
        **kwargs: Any,
    ) -> Result[dict, ToolError]:
        print("    ⏱️  [SpecSlow10sTool] Starting 10s background execution...")
        await asyncio.sleep(10.0)
        print("    ⏱️  [SpecSlow10sTool] Finished 10s background execution!")
        return Ok({"status": "slow_done"})


class MultiToolSpecDispatchSystem:
    def __call__(self, world: World) -> list[Event]:
        out: list[Event] = []
        for agent_id, view in world.query_agents(FSMSpecOrderComponent):
            comp = view.get_component(FSMSpecOrderComponent)
            if comp is None:
                continue

            last_type = view.domain_phase
            if last_type == "fsm.transitioned" and comp.stage == "processing":
                corr = correlation_middleware.current()
                principal = "tenant-a.agent-1"
                out.extend(
                    [
                        Event.create(
                            event_type="tool.ex29_fast_success_tool.requested",
                            agent_id=agent_id,
                            event_class="domain",
                            data={"step": "1"},
                            correlation=corr,
                            producer_principal_id=principal,
                        ),
                        Event.create(
                            event_type="tool.ex29_fast_failure_tool.requested",
                            agent_id=agent_id,
                            event_class="domain",
                            data={"step": "2"},
                            correlation=corr,
                            producer_principal_id=principal,
                        ),
                        Event.create(
                            event_type="tool.ex29_slow_10s_tool.requested",
                            agent_id=agent_id,
                            event_class="domain",
                            data={"step": "3"},
                            correlation=corr,
                            producer_principal_id=principal,
                        ),
                    ]
                )

        return out


# Transition guard combining built-in and custom specifications with .and_() and .or_()
spec_multi_tool_guard = (
    DomainStateIs("stage", "created").or_(DomainStateIs("stage", "draft"))
).and_(OrderAmountValid(min_amount=10, max_amount=1000))

multi_tool_spec_fsm_config = FSMConfig(
    component_type=FSMSpecOrderComponent,
    state_field="stage",
    transitions={
        "created": {
            "order.submit": FSMTransition(
                to="processing",
                guard=spec_multi_tool_guard,  # <--- Specification Guard
            ),
        },
        "processing": {
            "tool.ex29_fast_success_tool.completed": FSMTransition(to="approved"),
            "tool.ex29_fast_failure_tool.failed": FSMTransition(to="rejected"),
            "tool.ex29_slow_10s_tool.failed": FSMTransition(to="rejected"),
        },
    },
    terminal=frozenset({"approved", "rejected"}),
)


# ===========================================================================
# SCENARIO 2: Interleaved Tools FSM Components & Specification Guard
# ===========================================================================


@domain_component("process.start")
@dataclass(frozen=True, slots=True)
class FSMSpecInterleavedComponent(DomainComponent):
    stage: str = "draft"
    order_amount: int = 150


@tool_worker(name="ex29_slow_3s_tool", max_concurrency=10, retries=0)
class SpecSlow3sTool:
    async def invoke(
        self,
        *,
        idempotency_key: str,
        state: str = "",
        **kwargs: Any,
    ) -> Result[dict, ToolError]:
        print("    🐢 [SpecSlow3sTool] Starting 3s background execution...")
        await asyncio.sleep(3.0)
        print("    🐢 [SpecSlow3sTool] Finished 3s execution!")
        return Ok({"status": "slow_3s_done"})


@tool_worker(name="ex29_fast_005s_tool", max_concurrency=10, retries=0)
class SpecFast005sTool:
    async def invoke(
        self,
        *,
        idempotency_key: str,
        state: str = "",
        **kwargs: Any,
    ) -> Result[dict, ToolError]:
        print("    ⚡ [SpecFast005sTool] Executed in ~0.05s!")
        await asyncio.sleep(0.05)
        return Ok({"status": "fast_005s_done"})


class InterleavedSpecDispatchSystem:
    def __call__(self, world: World) -> list[Event]:
        out: list[Event] = []
        for agent_id, view in world.query_agents(FSMSpecInterleavedComponent):
            comp = view.get_component(FSMSpecInterleavedComponent)
            if comp is None:
                continue

            last_type = view.domain_phase
            if last_type == "fsm.transitioned" and comp.stage == "executing":
                corr = correlation_middleware.current()
                principal = "tenant-a.agent-1"
                out.extend(
                    [
                        Event.create(
                            event_type="tool.ex29_slow_3s_tool.requested",
                            agent_id=agent_id,
                            event_class="domain",
                            data={"job": "slow"},
                            correlation=corr,
                            producer_principal_id=principal,
                        ),
                        Event.create(
                            event_type="tool.ex29_fast_005s_tool.requested",
                            agent_id=agent_id,
                            event_class="domain",
                            data={"job": "fast"},
                            correlation=corr,
                            producer_principal_id=principal,
                        ),
                    ]
                )

        return out


interleaved_spec_fsm_config = FSMConfig(
    component_type=FSMSpecInterleavedComponent,
    state_field="stage",
    transitions={
        "draft": {
            "process.start": FSMTransition(
                to="executing",
                guard=OrderAmountValid(10, 500),  # <--- Custom Specification Guard
            ),
        },
        "executing": {
            "tool.ex29_fast_005s_tool.completed": FSMTransition(
                to="partially_completed",
                guard=DomainStateIs(
                    "stage", "executing"
                ),  # <--- Built-in Specification Guard
            ),
            "tool.ex29_slow_3s_tool.completed": FSMTransition(to="fully_completed"),
        },
        "partially_completed": {
            "tool.ex29_slow_3s_tool.completed": FSMTransition(
                to="fully_completed",
                guard=DomainStateIs("stage", "partially_completed"),
            ),
        },
    },
    terminal=frozenset({"fully_completed"}),
)


# ===========================================================================
# DRIVER FUNCTIONS
# ===========================================================================


async def run_scenario_1(redis) -> None:
    print("\n-------------------------------------------------------------------------")
    print("SCENARIO 1: Specification-Guarded FSM Parallel Fast-Fail")
    print("-------------------------------------------------------------------------")

    event_log = EventLog(RedisEventLogAdapter(client=redis))
    tool_router = ToolRouter(redis)

    concordo = BusinessFSMConcordo(multi_tool_spec_fsm_config)

    dispatcher = ReactiveDispatcher(
        log=event_log,
        systems=[MultiToolSpecDispatchSystem()],
        projections=[],
        redis=redis,
        tool_router=tool_router,
        poll_interval=0.01,
        rediscovery_interval_seconds=0.1,
        heartbeat_interval_seconds=0.0,
    )
    concordo.install(dispatcher)

    worker_manager = WorkerManager(
        redis=redis,
        event_log=event_log,
        reaper_interval=5.0,
        reaper_idle_time=15.0,
    )
    worker_manager.register(SpecFastSuccessTool, acl=None)
    worker_manager.register(SpecFastFailureTool, acl=None)
    worker_manager.register(SpecSlow10sTool, acl=None)

    agent_id = "fsm-spec-fast-fail-001"

    print(f"🚀 [0.0s] Triggering 'order.submit' for agent '{agent_id}'...")
    correlation_middleware.start(metadata={"scenario": "fsm_spec_fast_fail"})
    corr = correlation_middleware.current()
    principal = "tenant-a.agent-1"

    await event_log.append(
        Event.create(
            event_type="agent.spawned",
            agent_id=agent_id,
            event_class="lifecycle",
            correlation=corr,
            data={"role": "fsm_spec_agent"},
            producer_principal_id=principal,
        )
    )
    await event_log.append(
        Event.create(
            event_type="order.submit",
            agent_id=agent_id,
            event_class="domain",
            correlation=corr,
            data={"stage": "created", "order_amount": 250},
            producer_principal_id=principal,
        )
    )

    dispatcher.track_agent(agent_id)
    correlation_middleware.clear()

    start_time = time.monotonic()
    await dispatcher.start()
    await worker_manager.start()

    rejected_elapsed: float | None = None

    for _ in range(40):
        events = await event_log.read(agent_id)
        elapsed = round(time.monotonic() - start_time, 2)
        for e in events:
            if e.event_type == "fsm.transitioned" and e.data.get("to") == "rejected":
                if rejected_elapsed is None:
                    rejected_elapsed = elapsed
                    print(
                        f"\n💥 [{elapsed}s] SPEC-GUARDED FSM TRANSITIONED TO REJECTED STATE FAST!"
                        f"\n    Transition: {e.data.get('from')} -> {e.data.get('to')}"
                        f"\n    Trigger: {e.data.get('trigger')}"
                    )
        if rejected_elapsed is not None:
            break
        await asyncio.sleep(0.05)

    await dispatcher.stop()
    await worker_manager.stop()

    print(
        f"\n✅ Scenario 1 Passed: Specification-guarded FSM rejected in {rejected_elapsed}s!"
    )


async def run_scenario_2(redis) -> None:
    print("\n-------------------------------------------------------------------------")
    print("SCENARIO 2: Specification-Guarded FSM Interleaved Tools")
    print("-------------------------------------------------------------------------")

    event_log = EventLog(RedisEventLogAdapter(client=redis))
    tool_router = ToolRouter(redis)

    concordo = BusinessFSMConcordo(interleaved_spec_fsm_config)

    dispatcher = ReactiveDispatcher(
        log=event_log,
        systems=[InterleavedSpecDispatchSystem()],
        projections=[],
        redis=redis,
        tool_router=tool_router,
        poll_interval=0.01,
        rediscovery_interval_seconds=0.1,
        heartbeat_interval_seconds=0.0,
    )
    concordo.install(dispatcher)

    worker_manager = WorkerManager(
        redis=redis,
        event_log=event_log,
        reaper_interval=5.0,
        reaper_idle_time=15.0,
    )
    worker_manager.register(SpecSlow3sTool, acl=None)
    worker_manager.register(SpecFast005sTool, acl=None)

    agent_id = "fsm-spec-interleaved-001"

    print(f"🚀 [0.0s] Triggering 'process.start' for agent '{agent_id}'...")
    correlation_middleware.start(metadata={"scenario": "fsm_spec_interleaved"})
    corr = correlation_middleware.current()
    principal = "tenant-a.agent-1"

    await event_log.append(
        Event.create(
            event_type="agent.spawned",
            agent_id=agent_id,
            event_class="lifecycle",
            correlation=corr,
            data={"role": "fsm_spec_agent"},
            producer_principal_id=principal,
        )
    )
    await event_log.append(
        Event.create(
            event_type="process.start",
            agent_id=agent_id,
            event_class="domain",
            correlation=corr,
            data={"stage": "draft", "order_amount": 150},
            producer_principal_id=principal,
        )
    )

    dispatcher.track_agent(agent_id)
    correlation_middleware.clear()

    start_time = time.monotonic()
    await dispatcher.start()
    await worker_manager.start()

    partial_at: float | None = None
    full_at: float | None = None

    for _ in range(70):
        events = await event_log.read(agent_id)
        elapsed = round(time.monotonic() - start_time, 2)
        for e in events:
            if e.event_type == "fsm.transitioned":
                to_state = e.data.get("to")
                if to_state == "partially_completed" and partial_at is None:
                    partial_at = elapsed
                    print(
                        f"\n⚡ [{elapsed}s] SPEC FSM TRANSITIONED TO 'partially_completed'!"
                    )
                elif to_state == "fully_completed" and full_at is None:
                    full_at = elapsed
                    print(
                        f"\n🐢 [{elapsed}s] SPEC FSM TRANSITIONED TO 'fully_completed'!"
                    )
        if full_at is not None:
            break
        await asyncio.sleep(0.05)

    await dispatcher.stop()
    await worker_manager.stop()

    delta = round(full_at - partial_at, 2) if (full_at and partial_at) else 0.0
    print(
        f"\n✅ Scenario 2 Passed: Specification-guarded FSM completed fast at {partial_at}s, {delta}s before slow tool!"
    )


async def main() -> None:
    import redis.asyncio as aioredis

    use_fake = os.environ.get("KNT_REDIS_FAKE", "0") == "1"
    if use_fake:
        from fakeredis.aioredis import FakeRedis

        redis = FakeRedis(decode_responses=False)
        print("ℹ️  Running with FakeRedis (in-memory)")
    else:
        password = os.environ.get("KNT_REDIS_PASSWORD", "redispassword")
        redis = aioredis.Redis(
            host="localhost",
            port=6379,
            password=password,
            db=15,
            decode_responses=False,
        )
        await redis.flushdb()
        print("ℹ️  Running with Real Redis (port 6379, db=15)")

    await run_scenario_1(redis)
    await run_scenario_2(redis)
    await redis.aclose()

    print("\n=========================================================================")
    print("    ALL SPECIFICATION-GUARDED FSM SCENARIOS COMPLETED SUCCESSFULLY!")
    print("=========================================================================\n")


if __name__ == "__main__":
    asyncio.run(main())
